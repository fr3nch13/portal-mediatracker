<?php 
// File: app/View/CustodyChains/admin_user.ctp
...
foreach ($custody_chains as $custody_chain)
{
	...
	$options = '';
	$options .= $this->Html->link(__('View'), array('controller' => 'uploads', 'action' => 'view', $custody_chain['Upload']['id'], 'admin' => false));
	if($custody_chain['Upload']['filename'])
	{
		$options .= $this->Html->link(__('Download'), array('controller' => 'uploads', 'action' => 'download', $custody_chain['Upload']['id'], 'admin' => false));
	}
	if(in_array(AuthComponent::user('role'), array('admin', 'reviewer')))
	{
		$options .= $this->Html->link(__('Delete'), array('action' => 'delete', $custody_chain['CustodyChain']['id'], 'admin' => false),array('confirm' => Configure::read('Dialogues.deletecoc')));
	}
	...
	$td[$i] = array(
		...
		$this->Html->link($custody_chain['Upload']['filename'], array('controller' => 'uploads', 'action' => 'download', $custody_chain['Upload']['id'], 'admin' => false)),
