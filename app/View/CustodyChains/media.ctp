<?php 
// File: app/View/CustodyChains/media.ctp

$page_options = array(
	$this->Html->link(__('Add to Chain of Custody'), array('action' => 'add', $this->params['pass'][0])),
);
...
foreach ($custody_chains as $custody_chain)
{
	...
	$options = '';
	$filename = $custody_chain['Upload']['filename'];
	if($custody_chain['Upload']['filename'])
	{
		...
		$filename = $this->Html->link($custody_chain['Upload']['filename'], array('controller' => 'uploads', 'action' => 'download', $custody_chain['Upload']['id']));
	}
	...
	$td[$i] = array(
		...
		$filename,
		$this->Html->link($tmp['User']['name'], array('controller' => 'users', 'action' => 'view', $tmp['User']['id'])),
		array(
			$options, 
			array('class' => 'actions'),
		),
